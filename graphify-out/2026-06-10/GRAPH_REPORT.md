...
4. `MiniAvatar()` - 15 edges
5. `useAuthStore` - 15 edges
6. `🎓 Student Tracking System — Design Document` - 15 edges
7. `Key Features` - 14 edges
8. `Student Tracking System` - 13 edges
9. `authenticate()` - 12 edges
10. `StatTile()` - 9 edges
...
Cohesion: 0.05
Nodes (29): AdminDashboard(), ManageUsers(), StudentDetail(), Login(), ROLES, ProtectedRoute(), DashboardLayout(), adminItems (+21 more)
...
Cohesion: 0.09
Nodes (24): bulkUpload, bulkUploadStorage, docStorage, docUpload, excelDateToJSDate(), parseDate(), ParsedRow, prisma (+16 more)
